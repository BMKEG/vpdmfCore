 ...
 * 
 * <p>
 * Description: Imports view specification files and builds a VPDMf model for a whole system. 
 * </p>
 * 
 * <p>
...
public class VPDMfParser {
	...
	public VPDMf buildAllViews(VpdmfSpec vpdmfSpec, 
		...
		return this.buildAllViews(vpdmfSpec, model, allSpecFiles, null);
	...
	public VPDMf buildAllViews(VpdmfSpec vpdmfSpec, 
			...
			List<String> solrViews) throws Exception {
		...
		while( vdIt.hasNext() ) {
			...
			if( viewSpec.getParent() != null &&  
				...
				ViewDefinition vdParent = top.getViews().get(viewSpec.getParent());
				
				if( vd == vdParent ) 
					throw new Exception("A view cannot be it's own parent: " + vd.getName());
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		Iterator<UMLclass> cIt = top.getUmlModel().listClasses().values().iterator();
		while( cIt.hasNext() ) {
			UMLclass c = cIt.next();
			if( c.getStereotype() != null && c.getStereotype().equals("proxy")) {
				throw new Exception( "Unmerged proxy class found in model:" + c.getClassAddress() );
			}
		}
		...
		while( vdIt.hasNext() ) {
			ViewDefinition vd = vdIt.next();
			Iterator<IndexElement> idxIt = vd.getIndexElements().values().iterator();
			IDXLOOP: while( idxIt.hasNext() )  {
				IndexElement idx = idxIt.next();
				
				PrimitiveDefinition pd = (PrimitiveDefinition) 
						vd.getSubGraph().getNodes().get( idx.getP() );
	...
	private void constructViewShellsForSystem(VPDMf top) throws Exception {
		...
		InputStream is = ClassLoader.getSystemResourceAsStream("edu/isi/bmkeg/vpdmf/vpdmfSystem/vpdmfSystem.xml");		
	...
	private void constructViewShellsFromResource(VPDMf top, String path) throws Exception {
	...
	/**
	 * Builds a 'view shell'. This consists of a basic construction of the primitives 
	 * @param m
	 * @throws Exception
	 */
	private void constructViewShell(ViewDefinition vd, UMLmodel m) throws Exception {
		...
		if (t.toLowerCase().equals("data")) {
			...
			throw new Exception("ViewSpec Definition type: " + t +", not recognized");
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		Iterator<IndexElementSpec> ieIt = vd.getSpec().getIndexElements().iterator();
		while( ieIt.hasNext() ) {
		...
		while( pvIt.hasNext() ) {
			...
			//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			if( pvSpec.getLookupView() != null && pvSpec.getLookupView().length() > 0 ) {
			...
			while( condIt.hasNext() ) {
				...
				if( sc.size() != 1 ) {
					throw new Exception("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
							...
							+ cName
							+ " in the condition "  
							+ condSpec.toString() + " is incorrect.\n" 
							+ "Please check the spelling of the class in the ViewSpec or in the Model.\n"
							+ "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
				}
	...
	private void buildViewsFromShells(VPDMf top) throws Exception {
		...
		while (vdIt.hasNext()) {
			...
			Iterator<SuperGraphNode> pdIt = vd.getSubGraph().getNodes().values()
					.iterator();
			while (pdIt.hasNext()) {
	 ...
	 * @throws Exception
	 */
	private void addClassesToModelForTerminology(String pkgPattern) throws Exception {
		...
		if( !m.listClasses().containsKey(
			...
			InputStream is = ClassLoader.getSystemResourceAsStream("edu/isi/bmkeg/terminology/terminology.xml");		
		...
		UMLclass termClass = m.listClasses().get("|.edu.isi.bmkeg.terminology.model.Term");
	...
	private void addTermElementsToSpecs(VPDMf top, String pkgPattern) throws Exception {
			
		UMLmodel m = this.model;
		UMLclass termClass = m.listClasses().get("|.edu.isi.bmkeg.terminology.model.Term");
		UMLclass ontClass = m.listClasses().get("|.edu.isi.bmkeg.terminology.model.Ontology");
		UMLclass personClass = m.listClasses().get("|.edu.isi.bmkeg.person.model.Person");

		Map<String, UMLclass> classesToInclude = top.getUmlModel().listClasses(pkgPattern); 
		...
		while( vdIt.hasNext() ) {
			...
			while( cIt.hasNext() ) {
				...
				while( aIt.hasNext() ) {
					...
					if( a.getBaseName().equals("term") && a.getType().equals(termClass) ) {
						tc = c;
						break;
					}
				}			
			...
			if( tc != null ){
				...
				PrimitiveLinkSpec plSpec = new PrimitiveLinkSpec(tc.getBaseName(), "term", "Term", "Term");
				...
				cSpec.setName("Term");
				pSpec.getClasses().add(cSpec);
				plSpec = new PrimitiveLinkSpec("Term", "ontology", "Ontology", "Ontology");
				pSpec.getPvLinks().add(plSpec);
				vSpec.getPrimitives().add(pSpec);
